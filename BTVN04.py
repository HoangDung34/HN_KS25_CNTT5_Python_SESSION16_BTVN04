...
def display_records(records):
    ...
    for index, record in enumerate(records, start=1):
        patient_id, name, birth_year, diagnosis = record.split("-", 3)

        print(f"{index}. [{patient_id:<5}] | {name:<20} | Năm sinh: {birth_year:<10} | Chẩn đoán: {diagnosis}")
...
def update_diagnosis(records):
    ...
    parts = records[index].split("-", 3)

    print(f"Tìm thấy bệnh nhân: {parts[1]}")
    print(f"Chẩn đoán hiện tại: {parts[3]}")

    new_diagnosis = input("Nhập chẩn đoán mới: ").replace("-", " ").strip().capitalize()
